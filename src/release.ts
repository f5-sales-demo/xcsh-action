...
export function normalizeVersion(value: string, lockedVersion: string): string {
  ...
  if (!VERSION_PATTERN.test(requested)) {
    throw new Error(
      "xcsh-version must be 'locked' or an exact semantic version such as v20.4.0",
    );
  }
...
export async function loadLock(
  ...
  if (!VERSION_PATTERN.test(raw.version) || !isRecord(raw.platforms)) {
    throw new Error(
      "xcsh.lock.json contains an invalid version or platform map",
    );
  }
  for (const [key, value] of Object.entries(raw.platforms))
    validateLockedAsset(key, value);
...
export async function resolveAsset(options: {
  ...
  if (version === lock.version) {
    ...
    asset = await resolveReleaseAsset(
      version,
      expectedNames,
      options.githubToken,
    );
...
async function resolveReleaseAsset(
  ...
  if (!response.ok) {
    throw new Error(
      `Unable to resolve xcsh ${version}: GitHub API returned ${response.status}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `GitHub returned invalid release metadata for xcsh ${version}`,
    );
  }
...
function digestForAsset(assets: GithubReleaseAsset[], name: string): string {
  ...
  if (matches.length !== 1 || typeof matches[0]?.digest !== "string") {
    throw new Error(
      `xcsh release asset ${name} is missing a unique SHA-256 digest`,
    );
  }
...
function validateLockedAsset(
  key: string,
  value: unknown,
): asserts value is LockedAsset {
  if (!isRecord(value))
    throw new Error(`xcsh.lock.json has an invalid ${key} entry`);
  for (const field of [
    "archiveName",
    "archiveSha256",
    "archiveEntryName",
    ...
    if (typeof value[field] !== "string")
      throw new Error(`xcsh.lock.json ${key}.${field} must be a string`);
  }
  normalizeSha256(value.archiveSha256 as string);
  normalizeSha256(value.binarySha256 as string);
  if (
    !new Set<unknown>(["tar.gz", "zip", "executable"]).has(value.archiveKind)
  ) {
