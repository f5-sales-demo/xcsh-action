...
export async function run(): Promise<void> {
  ...
  if (!workingDirectoryMetadata.isDirectory()) {
    throw new Error(`working-directory is not a directory: ${inputs.workingDirectory}`);
  }
  ...
  const runDirectory = await mkdtemp(path.join(process.env.RUNNER_TEMP ?? os.tmpdir(), 'xcsh-action-'));
  const manifestFiles = await expandManifestFiles(inputs.fileEntries, inputs.workingDirectory);
  ...
  if (inputs.exportFile) {
    inputs.exportFile = path.resolve(inputs.workingDirectory, inputs.exportFile);
    await mkdir(path.dirname(inputs.exportFile), { recursive: true });
  }
  const arguments_ = buildXcshArguments(inputs, manifestFiles, internalResultFile);
  ...
  try {
    report = parseReport(await readFile(internalResultFile, 'utf8'), inputs.operation);
  } catch (error) {
    throw new Error(
  ...
  await publishOutputs(report, publishedResultFile, installed.path, installed.version);
  await publishSummary(report, installed.version);

  if (processResult.exitCode !== 0 || !report.success) {
    throw new Error(`xcsh ${inputs.operation} failed (${report.counts.failed.toString()} failed)`);
  }
}
...
async function publishOutputs(
  ...
  for (const status of STATUS_OUTPUTS) core.setOutput(status, count(report, status).toString());
  core.setOutput('xcsh-version', version);
  core.setOutput('xcsh-path', executablePath);
}

async function publishSummary(report: ResourceOperationReport, version: string): Promise<void> {
  if (!process.env.GITHUB_STEP_SUMMARY) return;
  const statusRows = Object.entries(report.counts)
    .filter(([key, value]) => !['total', 'succeeded', 'failed'].includes(key) && value > 0)
  ...
  core.summary
    ...
    .addTable([
      ...
      [report.counts.total.toString(), report.counts.succeeded.toString(), report.counts.failed.toString()],
    ]);
