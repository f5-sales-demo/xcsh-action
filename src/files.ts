...
export async function expandManifestFiles(
  entries: string[],
  ...
  for (const entry of entries) {
    if (entry === "-") {
      throw new Error(
        "files does not accept stdin ('-'); use the manifest input for inline JSON or YAML",
      );
    }
    if (entry.startsWith("!")) {
    ...
    if (matches.length === 0)
      throw new Error(`Manifest glob did not match any paths: ${entry}`);
