...
export async function installXcsh(
  asset: ResolvedAsset,
): Promise<InstalledXcsh> {
  const cacheVersion = asset.version.replace(/^v/u, "");
  const executableName = asset.platformKey.startsWith("win32-")
    ? "xcsh.exe"
    : "xcsh";
  const cachedDirectory = toolCache.find(
    "xcsh",
    cacheVersion,
    asset.platformKey,
  );
  if (cachedDirectory) {
    ...
    core.info(
      `Using verified xcsh ${asset.version} from the runner tool cache`,
    );
    return { path: cachedPath, version: asset.version, cacheHit: true };
  }

  const temporaryDirectory = await mkdtemp(
    path.join(process.env.RUNNER_TEMP ?? os.tmpdir(), "xcsh-install-"),
  );
  try {
    ...
    const releasedBinary = await extractReleasedBinary(
      archivePath,
      temporaryDirectory,
      asset,
    );
    ...
    if (!asset.platformKey.startsWith("win32-"))
    ...
    const cacheDirectory = await toolCache.cacheDir(
      stagingDirectory,
      "xcsh",
      cacheVersion,
      asset.platformKey,
