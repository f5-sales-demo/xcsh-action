...
const MANIFEST_OPERATIONS = new Set(['apply', 'create', 'update', 'diff', 'validate']);
const DRY_RUN_OPERATIONS = new Set(['apply', 'create', 'update', 'delete']);

export function buildXcshArguments(inputs: ActionInputs, manifestFiles: string[], resultFile: string): string[] {
  validateCommandInputs(inputs, manifestFiles);
  const arguments_: string[] = [inputs.operation];

  if ((inputs.operation === 'get' || inputs.operation === 'delete') && manifestFiles.length === 0) {
  ...
  if (inputs.operation === 'export' && inputs.exportFile) arguments_.push('-f', inputs.exportFile);
  return arguments_;
}

export function validateCommandInputs(inputs: ActionInputs, manifestFiles: string[]): void {
  const hasFiles = manifestFiles.length > 0;
  if (MANIFEST_OPERATIONS.has(inputs.operation) && !hasFiles) {
    throw new Error(`${inputs.operation} requires at least one manifest from files or manifest`);
  }
  ...
  if (inputs.operation === 'delete' && !hasFiles) {
    if (!inputs.resourceKind || !inputs.resourceName)
      throw new Error('delete requires manifests or both resource-kind and resource-name');
  }
  if (hasFiles && (inputs.resourceKind || inputs.resourceName)) {
    throw new Error('resource-kind and resource-name cannot be combined with manifest files');
  }
  if (inputs.operation === 'export') {
    ...
    if (inputs.resourceName && !inputs.resourceKind) throw new Error('resource-name requires resource-kind');
