...
test("expands include globs, keeps directories, and removes duplicates", async () => {
  ...
  assert.deepEqual(
    await expandManifestFiles(
      ["manifests/*.yaml", "manifests/a.yaml"],
      directory,
    ),
    [path.join(directory, "manifests", "a.yaml")],
  );
  assert.deepEqual(await expandManifestFiles(["manifests"], directory), [
...
test("rejects stdin and unmatched globs", async () => {
  ...
  await assert.rejects(
    expandManifestFiles(["-"], directory),
    /manifest input/u,
  );
  await assert.rejects(
    expandManifestFiles(["*.yaml"], directory),
    /did not match/u,
  );
});
