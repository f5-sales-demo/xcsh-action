...
import { normalizeSha256, sha256File, verifyFileSha256 } from '../src/digest.js';
...
test('hashes and verifies downloaded bytes', async () => {
  ...
  const digest = '22b5658643265117773235934c1b8903f1329debe2d776d742e3fcb399c08f64';
  assert.equal(await sha256File(file), digest);
  await verifyFileSha256(file, `sha256:${digest}`);
  await assert.rejects(verifyFileSha256(file, '0'.repeat(64)), /SHA-256 mismatch/u);
});
