...
test('builds repeated file arguments without interpreting shell characters', () => {
  ...
  assert.deepEqual(buildXcshArguments(inputs(), ['/workspace/a.yaml', suspicious], '/tmp/result.json'), [
    'apply',
    '-f',
    '/workspace/a.yaml',
    '-f',
...
test('builds export all with a manifest output file', () => {
  assert.deepEqual(
    ...
    ['export', '--all', '-o', 'yaml', '--result-file', '/tmp/result.json', '-f', 'export.yaml'],
  );
});

test('builds a resource-kind get that lists resources', () => {
  assert.deepEqual(
    buildXcshArguments(inputs({ operation: 'get', resourceKind: 'origin_pool' }), [], '/tmp/result.json'),
    ['get', 'origin_pool', '-o', 'table', '--result-file', '/tmp/result.json'],
  );
});

test('rejects missing manifests and invalid operation-specific inputs', () => {
  assert.throws(() => buildXcshArguments(inputs(), [], '/tmp/result.json'), /requires at least one manifest/u);
  assert.throws(
    () => buildXcshArguments(inputs({ operation: 'diff', dryRun: 'client' }), ['a.yaml'], '/tmp/result.json'),
